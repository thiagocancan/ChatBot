...
$query = "SELECT u.username, COUNT(m.id) as message_count 
          ...
          WHERE m.role = 'model' 
          GROUP BY u.id";
$messageStats = $db->query($query)->fetchAll();

// 2. Get daily access count (from access_logs table)
$today = date('Y-m-d');
$query = "SELECT COUNT(*) as count FROM access_logs WHERE DATE(access_time) = ?";
$accessCount = $db->query($query, [$today])->fetch()['count'] ?? 0;
...
$query = "SELECT m.content, m.role, u.username, m.created_at 
          ...
          LIMIT 20";
$recentMessages = $db->query($query)->fetchAll();
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
